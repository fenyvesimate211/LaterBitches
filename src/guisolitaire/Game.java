...
class Game {
    //Constants
	private final double CARD_WIDTH = 150, CARD_HEIGHT = 240,
		ROUNDING_FACTOR = 10, PADDING = 25;	

	//Variables used to store the guisolitaire.cards and bounds for the game
	private final Stack<Card> hand = new Stack<>(), waste = new Stack<>();
	private final List<Stack<Card>> board = new ArrayList<>(), foundations = new ArrayList<>();
	private final BoundingBox handBounds = new BoundingBox(PADDING, PADDING, CARD_WIDTH, CARD_HEIGHT), wasteBounds = new BoundingBox(PADDING * 2 + CARD_WIDTH, PADDING, CARD_WIDTH, CARD_HEIGHT);
	...
	/**
	 * Lay out the board from the pack of guisolitaire.cards. Essentially places
	 * sequentially increasing stacks of guisolitaire.cards on each space of the board
	 */
	...
	/**
	 * Place all the guisolitaire.cards in the waste back into the pack.
	 */
	...
	private void loadImages() {
		for (Suit suit : Suit.values()) {
			for (Value value : Value.values()) {
				...
				imageCache.put(filename, new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("/src/guisolitaire/cards/" + filename + ".png"))));
			}
		}
		imageCache.put("cardback", new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("/src/guisolitaire/cards/cardback.png"))));
	}
...
//	}
    ...
    /**
     * Reveal any guisolitaire.cards which are not yet revealed and on the bottom of their board stack
     */
